methods {
    function getStatus(PoolIdLibrary.PoolId poolId) external returns (PoolStatusManager.PoolStatus memory) envfree;
...
returns (uint256,uint24,uint256) {
    ...
    uint24 fee = dynamicFeeCVL(timestamp, status.marginTimestampLast, zeroForOne ? reserveIn : reserveOut, zeroForOne ? reserveOut : reserveIn);
    /* Non-deterministic approach (unconstrained) */
    //uint256 fee; require fee < MAX_FEE_UNITS();
    return amountInCVL(amountOut, reserveOut, reserveIn, fee), fee, amountInFeeCVL(amountOut,reserveOut,reserveIn);
}
...
returns (uint256,uint24,uint256) {
    ...
    return return amountOutCVL(amountIn, reserveIn, reserveOut, fee), fee, amountOutFeeCVL(amountIn,reserveIn,reserveOut);
}*/

/// Ghost summary for MarginFees.dynamicFee(status) - assumes dependency on four parameters only.
ghost dynamicFeeCVL(uint256 /* timestamp */, uint256 /* last timestamp */, uint256 /* reserve0 */, uint256 /* reserve1 */) returns uint24 {
...
ghost amountInFeeCVL(uint256,uint256,uint256) returns uint256;
ghost amountOutCVL(uint256 /* amountIn */, uint256 /* reserveIn */, uint256 /* reserveOut */ , uint256 /* fee */) returns uint256;
ghost amountOutFeeCVL(uint256,uint256,uint256) returns uint256;